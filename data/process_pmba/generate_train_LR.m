...
for idx_set = 1:length(dataset)
    ...
    for idx_im = 1:length(filepaths)
        ...
        for scale = scale_all
            ...
            % fn
            fn_HR = fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], [name_im(1:end-4), '_x', num2str(scale), '.png']);
            fn_LR = fullfile(['./LR/LR', degradation], dataset{idx_set}, ['x', num2str(scale)], [name_im(1:end-4), '_x', num2str(scale), '.png']);
